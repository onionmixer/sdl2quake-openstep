...
# attached from another session.  WARP off; $2 = dump every N FRAMES (default 1).
BIN=${1:-/usr/local/nxbuild/bin/glquake_g}
cd /usr/local/quake || exit 1
OSMGA_MESA_WARP=0; export OSMGA_MESA_WARP
OSMGA_STATS_EVERY=${2:-1}; export OSMGA_STATS_EVERY
OSMGA_MESA_REFUSAL_LIMIT=${3:-8}; export OSMGA_MESA_REFUSAL_LIMIT
